...
>
> **Codex's findings are not withdrawn — the code they were about is.** B-1 and
> B-5's fixes stand. B-2, B-4, B-6 and B-8 concerned the readiness marker, the
> set-aside removal and the durable inventory, all of which D-32 deleted. B-8's
> *finding* is therefore live again by design and is disclosed rather than fixed;
> see the D-32 banner text above for where that is recorded.
