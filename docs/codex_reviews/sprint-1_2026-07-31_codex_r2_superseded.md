> **SUPERSEDED.** Two versions of the round-2 verdict were pushed. The
> authoritative one is `sprint-1_2026-07-31_codex_r2.md` (commit `8dcbbbb`),
> and it has materially different findings. This earlier version is retained
> because four of its findings were real and were fixed — the enumeration
> failure, the `recursive=False` drop, the resume key omitting `limits`, and
> the float deadlines — and the record should show where that work came
> from. It also reached the OPPOSITE conclusion on hashing `terminal_status`;
> the authoritative verdict's position is the one that governs.
