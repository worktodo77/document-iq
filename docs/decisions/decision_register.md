...
low-confidence OCRs… the files OCR'd are in good shape and it's not apparent
why."* Full measurement: `docs/verification/ocr_threshold_2026-08-18.md`.

| # | Decision | Ruling | Date |
|---|---|---|---|
| D-44 | The review flag disagreed with itself, fired at an uncalibrated threshold, and fired on blank pages | **FIX ALL THREE (Alex).** (a) ONE predicate — `contracts.needs_ocr_review` — used by the log, the screen and the extractor, deciding on the ROUNDED percent, because that is the number displayed and recorded. The screen compared the raw float and the log the rounded percent, so 19 pages at confidences like 84.73% were shown to the operator and **left out of the audit record**. (b) The default threshold moves **85 → 80**, measured: over all 377 OCR pages the distribution is ONE population (median 86.3%, modal band 85-89 holding 236 pages) and 85 sat on that band's left edge, flagging 26.3%. D-19 had already measured the engine's own mean at 86.28% on this corpus. 80 is where the distribution breaks. (c) A page with almost no text AND almost no lines is excluded from review and **counted in the log** as `ocr_pages_without_usable_text` — 11 flagged pages carried under 20 characters, with confidences repeating exactly across documents, which is one speck of noise read as one token. Combined effect on the run that raised it: **screen 99 / log 80 → 11 / 11.** | 2026-08-18 |
| D-45 | Tesseract, which D-19 wrote off | **BENCHMARK IT (Alex) — D-19's cancellation is reversed in fact.** D-19 said the Tesseract evaluation was *"cancelled, not deferred: there is no pending Tesseract evaluation, and no future sprint owes one."* Alex authorized the install and the benchmark; Tesseract 5.4.0 and pytesseract are now on this machine. **Result: not a rout, and the finding is language-shaped.** On 61 flagged real-text pages Tesseract returned >10% more text on 13 pages and rapidocr on 4; on Portuguese prose rapidocr collapses word boundaries into unusable runs where Tesseract reads correctly, and on some graphic pages rapidocr is the better of the two. **The decisive number is the failure RATE by language: English 10.9%, Portuguese 43.7%.** On English material — 68.7% of this corpus and the majority of expected work — the shipped engine is performing acceptably and the engine is not the problem. **No engine change is ruled.** What is recorded is that the engine question belongs to non-English material, and that any future benchmark must run on a genuinely English corpus: the 61 pages were selected for failure and were 77% Portuguese, so they cannot be read as a sample of anything. | 2026-08-18 |

**One methodological correction worth carrying.** The first language reading was
taken over the FLAGGED pages and reported Portuguese-dominant. Alex challenged
it — *"most of the corpus is english text, is it not?"* — and he was right: the
corpus is 68.7% English. The flagged set is **selected for failure**, so it
over-represents whatever fails, and reading it as a sample of the corpus inverted
the conclusion. The engine looked broken for English when it is not.
